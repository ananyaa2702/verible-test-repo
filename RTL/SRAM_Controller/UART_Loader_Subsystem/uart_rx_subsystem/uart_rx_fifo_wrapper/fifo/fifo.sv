...
// Description:
// Top-level synchronous FIFO wrapper. Instantiates fifo_ctrl and fifo_mem,
// exposes FWFT or registered read data, and surfaces pointer/full/empty status
// for flow control.
//////////////////////////////////////////////////////////////////////////////////
...
module fifo #(
  ...
  /*
  WFT (First-Word Fall-Through) is a FIFO mode that makes the first data
  word immediately available on the output without requiring a read enable
  signal.

  FWFT = 1 (enabled): o_rd_data = mem_rd_data continuously
  FWFT = 0 (disabled): o_rd_data = i_rd_en ? mem_rd_data : '0
  (data only valid when reading)
  */
