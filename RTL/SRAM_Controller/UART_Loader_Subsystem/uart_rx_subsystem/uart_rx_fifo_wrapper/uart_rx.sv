...
// Description:
// Oversampling UART receiver. Implements a reset-safe FSM that detects start bits,
// accumulates data bits into a shift register using three-sample majority voting,
// optionally checks parity, validates stop bits, asserts write strobes toward an
// external FIFO, and raises sticky error flags on framing violations.
//////////////////////////////////////////////////////////////////////////////////
...
module uart_rx #(
  ...
  localparam CyclesPerBit    = SystemClockFreq / BaudRate; // 100_000_000 / 115200 = 868
  ...
  localparam ClkCounterWidth = $clog2(CyclesPerBit); // clog2(868) = 10
  localparam BitCounterWidth = $clog2(DataLength); // clog2(8) = 3
  ...
  always_comb begin
    if (clk_counter == (CyclesPerBit / 2 - TenthOfBit)) begin // 868/2 - 87 = 434 - 87 = 347
      strobe_bit   = 1'b0;
      sample_bits  = 2'd1; // Sample first bit for three
      half_bit     = 1'b0;
    end else if (clk_counter == CyclesPerBit / 2) begin // 868/2 = 434
      strobe_bit   = 1'b0;
      sample_bits  = 2'd2; // Sample second bit for three
      half_bit     = 1'b1;
    end else if (clk_counter == (CyclesPerBit / 2 + TenthOfBit)) begin // 868/2 + 87 = 434 + 87 = 521
      strobe_bit   = 1'b0;
      sample_bits  = 2'd3; // Sample the last bit for maj 3
      half_bit     = 1'b0;
    end else if (clk_counter == CyclesPerBit - 1) begin // 868 - 1 = 867
      strobe_bit   = 1'b1;
      sample_bits  = '0;
      half_bit     = 1'b0;
    end else begin
  ...
  always_ff @(posedge i_clk, negedge i_rst_n)
    if (!i_rst_n)
      ...
      /*
      majority_three(three_bits) - Returns the majority vote (0 or 1) from
      three samples of the RX line, providing noise immunity
      o_rx_data[DataLength-1:1] - Takes the upper 7 bits of the
      current data (bits [7:1] when DataLength=8)
      {..., ...} - Concatenates the new bit at the MSB position
      with the shifted-down existing bits
      */
  ...
  always_ff @(posedge i_clk, negedge i_rst_n)
    if (!i_rst_n)
      bit_counter <= DataLength - 1; // 8 - 1 = 7
