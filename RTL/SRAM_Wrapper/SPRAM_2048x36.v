...
`celldefine
module SPRAM_2048x36(A,CE,WEB, OEB, CSB,
                    I,O);
...
SPRAM_2048x36_behave ram (a_state,ck_state,web_state,oeb_state,csb_state,
                    i_state,o_state,
...
specify
  specparam DF     = 1.0,
            tACC  = 2.381*DF, // access time
            tCYC   = 2.828*DF,   // cycle time
            tHCEB  = 0.9531*DF,  // minimum clock high time.
	    tLCEB  = 0.4752*DF,  // minimum clock low time.
            tWSr   = 0.136*DF,   // WEB rising setup time
            tWSf   = 0.131*DF,   // WEB falling setup time
            tIS    = 0.5269*DF,   // Data setup time
	    tWH    = 0.079*DF,    // WEB hold time
	    tAS    = 0.4656*DF,    // Address setup time
	    tAH    = 0*DF,    // Address hold time
	    tIH    = 0*DF,    // Data hold time
	    tCSf   = 0.9531*DF,    // CSB falling setup time
	    tCSr   = 0.141*DF,    // CSB rising setup time
	    tCH    = 0*DF,    // CSB hold time
	    tHOE   = 0.618*DF,   // OEB disable time
	    tLOE   = 0.6242*DF,  // OEB enable time
	    tLBA   = 0.8003*DF,   // BA disable time
	    tHBA   = 0.7918*DF,  // BA enable time
	    tHW    = 0.5221*DF,    // WEB disable time
	    tLW    = 0.6025*DF;   // WEB enable time
            // PATHPULSE$CE$O = 0,
            // PATHPULSE$OEB$O = 0;
  ...
  $setup(posedge A[10],posedge CE &&& enable_sh,tAS,sh_a_error);
  $setup(negedge A[10],posedge CE &&& enable_sh,tAS,sh_a_error);
  $hold(posedge CE &&& enable_sh,posedge A[10],tAH,sh_a_error);
  $hold(posedge CE &&& enable_sh,negedge A[10],tAH,sh_a_error);
...
module SPRAM_2048x36_behave(a_state,ck_state,web_state,oeb_state,csb_state,
                    i_state,o_state,
